...
> 文档状态：UE Story Consumer 迁移准备文档。当前随主仓库维护，后续应随 `NarrRail/` 与 `NarrRailUEHost/` clean copy 到 `NarrRail-Unreal-Plugin`。
...
- `Docs/02_runtime/SCRIPT_FORMAT.md` 是运行时侧历史入口
...
## 4. Setup from the current main repository

Until `NarrRail-Unreal-Plugin` exists, the plugin and sample host are still stored in the main repository:
