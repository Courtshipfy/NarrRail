...
6. 将故事仓库交付给声明兼容的 Story Consumer
...
NarrRailEditor 负责创作、校验、预览和导出 `.nrstory` / `.nroutline`。下游 Story Consumer 读取这些文件，并声明与主仓库中立格式契约的兼容版本。

Unreal Engine 插件和示例工程已经迁移到独立仓库：

<https://github.com/Courtshipfy/NarrRail-Unreal-Plugin>

主仓库不再维护 UE 侧 Sync Stories、Blueprint、PIE、资产映射或 Host 项目说明；这些实现细节由外部 Unreal Story Consumer 仓库维护。
